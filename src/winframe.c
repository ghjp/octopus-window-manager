...
void wframe_list_add(gswm_t *gsw, wframe_t *frame)
{
  ...
  if(G_UNLIKELY(c->w_type.desktop)) {
    if(!g_list_find(scr->desktop_frlist, frame))
      scr->desktop_frlist = g_list_append(scr->desktop_frlist, frame);
  }
...
void wframe_list_remove(gswm_t *gsw, wframe_t *frame)
{
  ...
  scr->desktop_frlist = g_list_remove(scr->desktop_frlist, frame);
...
static void _place_client_to_monitor_warea(gswm_t *gsw, client_t *c)
{
  ...
  xmax = -bw + MIN(mon_rect.x2, wa->x + wa->w);
  ymax = -bw + MIN(mon_rect.y2, wa->y + wa->h);
  if(c->x + c->width > xmax)
    c->x = xmax - c->width;
  if(c->x < xmin)
    c->x = xmin;
...
static void _init_position(gswm_t *gsw, client_t *c)
{
  ...
  if((!c->w_type.dialog && USPosition & c->xsize.flags) || /* xine menu */
      ((c->w_type.toolbar || c->w_type.menu || c->w_type.kde_override) && (USPosition|PPosition) & c->xsize.flags) /* Google earth bubble windows */) {
    c->x = c->xsize.x;
    c->y = c->xsize.y;
    fix_ewmh_position_based_on_struts(gsw, c);
